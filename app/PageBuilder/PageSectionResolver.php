    ...
    public function resolve(AcademyPage $page, AcademyPageSection $section): array
    {
        ...
        if (in_array($section->type, ['hero', 'instructor', 'course', 'curriculum', 'pricing'], true) && $courseId > 0) {
            ...
            if ($course) {
                ...
                $studentCount = (int) $course->enrollments_count;
                ...
                if ($section->type === 'pricing') {
                    $offerIds = collect((array) ($settings['offer_ids'] ?? []))
                        ->map(fn ($id) => (int) $id)
                        ->filter()
                        ->values();
                    $offers = CourseOffer::query()
                        ->where('course_id', $course->id)
                        ->where('is_active', true)
                        ->when($offerIds->isNotEmpty(), fn ($query) => $query->whereIn('id', $offerIds))
