...
type PageProps = {
    course: Course;
    categories: Category[];
    academy?: { features?: Record<string, boolean> };
};

export default function CourseEdit() {
    const { course, categories, academy } = usePage<PageProps>().props;
    const features = academy?.features ?? {};
    ...
    const generateCourseImage = (purpose: 'cover' | 'thumbnail') => {
        const label = purpose === 'cover' ? 'cover' : 'thumbnail';
        const prompt = window.prompt(
            `Décris la ${label} à générer pour « ${course.title} »`,
            `${course.title}, visuel premium de formation, cohérent avec la marque de l’académie`,
        );
        if (!prompt?.trim()) return;

        setGeneratingMedia(purpose);
        router.post(
            `/trainer/courses/${course.id}/generate-image`,
            { purpose, prompt: prompt.trim() },
            {
                preserveScroll: true,
                onFinish: () => setGeneratingMedia(null),
            },
        );
    };

    const generateLessonAudio = (lesson: Lesson) => {
        if (!lesson.id) return;
        setGeneratingMedia(`audio-${lesson.id}`);
        router.post(
            `/trainer/lessons/${lesson.id}/generate-audio`,
            {},
            {
                preserveScroll: true,
                onFinish: () => setGeneratingMedia(null),
            ...
            <div className="mx-auto w-full max-w-[1180px] space-y-6 px-4 py-5 sm:px-6 sm:py-7 lg:px-8">
                <div className="academy-surface rounded-[calc(var(--radius)*1.25)] p-6 sm:p-7">
                    ...
                    <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
                        Ajustez le contenu, la structure et les paramètres de
                        votre formation sans changer de workflow.
                    </p>
                    <div className="mt-5 flex flex-wrap gap-2">
                        {features.assessments ? (
                            <Button asChild variant="outline">
                                <Link href={`/trainer/courses/${course.id}/assessments`}>Quiz & évaluations</Link>
                            </Button>
                        ) : null}
                        {features.assignments ? (
                            <Button asChild variant="outline">
                                <Link href={`/trainer/courses/${course.id}/assignments`}>Assignments & projets</Link>
                            </Button>
                        ) : null}
                        {features.completion ? (
                            <Button asChild variant="outline">
                                <Link href={`/trainer/courses/${course.id}/completion`}>Complétion & certificats</Link>
                            </Button>
                        ) : null}
                        {features.drip ? (
                            <Button asChild variant="outline">
                                <Link href={`/trainer/courses/${course.id}/learning-access`}>Drip & prérequis</Link>
                            </Button>
                        ) : null}
                    </div>
                </div>
                        ...
                        <>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <textarea id="target_audience" name="target_audience" rows={3} defaultValue={course.target_audience ?? ''} className="flex min-h-[72px] w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
                                </div>

                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-2">
                                        <Label htmlFor="level">Niveau</Label>
                                        <select id="level" name="level" defaultValue={course.level ?? 'all_levels'} className="flex h-10 w-full rounded-md border border-input bg-background px-3 text-sm">
                                    ...
                                    <div className="space-y-2"><Label htmlFor="language">Langue</Label><Input id="language" name="language" defaultValue={course.language ?? 'fr'} /></div>
                                </div>

                                <div className="grid gap-4 sm:grid-cols-2">
                                    <div className="space-y-2"><Label>Problème principal</Label><Input name="positioning[main_problem]" defaultValue={course.positioning?.main_problem ?? ''} /></div>
                                    <div className="space-y-2"><Label>Transformation souhaitée</Label><Input name="positioning[desired_transformation]" defaultValue={course.positioning?.desired_transformation ?? ''} /></div>
                                    <div className="space-y-2"><Label>Promesse principale</Label><Input name="positioning[main_promise]" defaultValue={course.positioning?.main_promise ?? ''} /></div>
                                    <div className="space-y-2"><Label>Angle unique</Label><Input name="positioning[unique_angle]" defaultValue={course.positioning?.unique_angle ?? ''} /></div>
                                </div>
                                ...
                                <div className="space-y-2 sm:col-span-2 lg:col-span-12">
                                    ...
                                    <div className="flex flex-wrap gap-2 pt-1">
                                        <Button
                                            ...
                                            onClick={() => generateCourseImage('cover')}
                                        >
                                            {generatingMedia === 'cover'
                                                ? 'Génération…'
                                                : 'Générer la cover'}
                                        </Button>
                                        <Button
                                            ...
                                            variant="outline"
                                            disabled={generatingMedia !== null}
                                            onClick={() => generateCourseImage('thumbnail')}
                                        >
                                            {generatingMedia === 'thumbnail'
                                                ? 'Génération…'
                            ...
                            <div className="space-y-4">
                                ...
                                <ModulesLessons
                                    ...
                                    generatingAudioLessonId={
                                        generatingMedia?.startsWith('audio-')
                                            ? Number(generatingMedia.replace('audio-', ''))
                                            : null
                                    }
                                />
                            </div>
...
CourseEdit.layout = (page: React.ReactNode) => (
    <CourseEditLayout>{page}</CourseEditLayout>
);
